...
export function meta({ params }: MetaArgs) {
  assertKeys(params, ["org_slug", "database_slug", "query_id"]);
  return [
    { title: `${params.database_slug} / ${params.query_id}` },
  ] satisfies MetaDescriptor[];
}

export async function loader({ request, params }: LoaderFunctionArgs) {
  assertResponseParams(params, ["org_slug", "database_slug", "query_id"]);
  ...
  const getQueryMessage = await dataFnMiddleware(
    request,
    client.v2.getQuery({
      params,
      headers,
    }),
  ).then(extractMessageOrThrow("get_query"));

  const { table, schema } =
    getQueryMessage.payload.query.type === "intent"
      ? getQueryMessage.payload.query.intent
      : {};
    ...
    schema: buildRunQueryIntentSchema({
      intent: {
        ...
        schema: z
          ...
          .default(
            getQueryMessage.payload.query.type === "intent"
              ? getQueryMessage.payload.query.intent.schema
              : "",
          ),
        table: z
          .string()
          .default(
            getQueryMessage.payload.query.type === "intent"
              ? getQueryMessage.payload.query.intent.table
              : "",
          ),
      },
    }),
  ...
  const query = (
    submission.status === "success"
      ? { ...getQueryMessage.payload.query, ...submission.value.query }
      : getQueryMessage.payload.query
  ) satisfies z.output<ReturnType<typeof buildRunQueryIntentSchema>>["query"];
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  switch (submission.value.intent) {
    ...
    case INTENTS.updateQuery: {
      const response = await dataFnMiddleware(
        request,
        client.v2.updateQuery({
          body: {
            query: submission.value.query,
          },
          headers,
      ...
      throw redirectDocument(
        `/orgs/${message.payload.query.org_slug}/databases/${message.payload.query.database_slug}/explorer/queries/${message.payload.query.id}`,
        {
          headers: await setFlashHeaders({
            type: "success",
            message: "This Query has been updated successfully.",
          }),
        },
      );
    }
