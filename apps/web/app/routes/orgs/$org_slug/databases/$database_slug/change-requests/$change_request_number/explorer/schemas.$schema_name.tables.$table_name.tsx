...
export const handle = {
  banner(match: UIMatch) {
    return (
      <FlashMessage
        title="Edit Change Request"
        description="Add or remove data changes below. Return to the Change Request after changes are completed by clicking Return."
        category="generalNotice"
        buttonGroup={
          <div className="flex gap-2">
            <LinkButton
              to={`/orgs/${match.params.org_slug}/databases/${match.params.database_slug}/change-requests/${match.params.change_request_number}/data-changes`}
            >
              Return
            </LinkButton>
          </div>
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (resultMessage.type !== "run_query") {
    throw redirect(
      `/orgs/${params.org_slug}/databases/${params.database_slug}/change-requests/${params.change_request_number}/explorer`,
      {
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  switch (submission.value.intent) {
    ...
    case INTENTS.updateChangeRequest: {
      const response = await client.v2.createChanges({
        body: submission.value.changes,
        headers,
        params,
      });
      ...
      if (message.type !== "create_changes") {
        return submission.reply(errorMessageToReplyOptions(message));
      }

      throw redirect(
        `/orgs/${params.org_slug}/databases/${params.database_slug}/change-requests/${params.change_request_number}/data-changes`,
        {
          headers: await setFlashHeaders({
            type: "success",
            message: "Change Request created successfully.",
...
export default function Route() {
  ...
  return (
    <DataExplorerTable
      ...
      changeRequestNumber={Number(params.change_request_number)}
